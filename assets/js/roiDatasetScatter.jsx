...
const ScatterGapCeiling = ({ murtyData, nsdData, ceilingData, roi, dataset, training }) => {
    ...
    const process = (rawData, source) => {
      ...
      return Object.entries(rawData).map(([key, stats]) => {
        ...
        return {
          ...
          gap: stats?.normalized_gap ?? 0, ceiling: stats?.ceiling_mean ?? 0, raw: stats
        };
  ...
  useEffect(() => {
    ...
    const centerXOffset = (plotAreaWidth - mainSize) / 2;
    ...
    const groupGap = 15;
    const targetROI = (roi === "Across Regions" || !roi) ? "" : String(roi).toLowerCase();
    const visibleROIs = Object.entries(roiColors).filter(([key]) => key !== 'overall' && (targetROI === "" || key === targetROI));
    const legendG = svg.append("g").attr("transform", `translate(20, 20)`);
    visibleROIs.forEach(([key, color], i) => {
        const row = legendG.append("g").attr("transform", `translate(0, ${i * rowHeight})`);
        row.append("circle").attr("r", 5).attr("fill", color);
        row.append("text").attr("x", 12).attr("y", 4).style("font-size", "10px").style("font-weight", "600").text(key.toUpperCase());
    });
    const shapeLegendY = visibleROIs.length * rowHeight + groupGap;
    const shapeLegend = legendG.append("g").attr("transform", `translate(0, ${shapeLegendY})`);
    Object.entries(datasetTypeLabels).forEach(([type, label], i) => {
        const row = shapeLegend.append("g").attr("transform", `translate(0, ${i * rowHeight})`);
        const shapeFn = type === "circle" ? d3.symbolCircle : type === "triangle" ? d3.symbolTriangle : d3.symbolDiamond;
        row.append("path").attr("d", d3.symbol().type(shapeFn).size(40)()).attr("fill", "#666").attr("transform", "translate(0, -1)");
        row.append("text").attr("x", 12).attr("y", 4).style("font-size", "10px").text(label);
    });
    ...
    // 绘制聚合点
    const dots = g.selectAll(".agg-dot")
      ...
      .attr("d", d => {
        const ds = d.id.split("/")[0];
        const shapeFn = datasetShapes[ds] || d3.symbolCircle;
        return d3.symbol().type(shapeFn).size(120)();
      })
      ...
      // ✨ 动态控制主点的不透明度
      .attr("opacity", d => {
        if (!activeInfo) return 1;
        return d.id === activeInfo.id ? 0.8 : 0.4; // 激活点本体变淡让位给裂变，其他点几乎看不见
      })
    ...
    // ✨ 绘制裂变逻辑 (渲染在 hoverLayer)
    if (activeInfo) {
      const ds = activeInfo.id.split("/")[0];
      const roiKey = activeInfo.id.split("/")[1];
      const shapeFn = datasetShapes[ds] || d3.symbolCircle;
      const color = roiColors[roiKey] || "#999";
      ...
      activeInfo.points.forEach((pt, i) => {
        ...
        hoverLayer.append("path")
          .attr("class", "fission-point")
          .attr("d", d3.symbol().type(shapeFn).size(100)())
          .attr("transform", `translate(${x(activeInfo.ceiling_mean)},${y(activeInfo.gap_mean)})`)
          .attr("fill", color)
          .attr("stroke", "black")
        ...
        hoverLayer.append("text")
          .attr("class", "fission-label")
          .attr("x", px)
          .attr("y", i === 0 ? py - 14 : py + 22)
          .attr("text-anchor", "middle")
    ...
    svg.append("text").attr("x", margin.left + centerXOffset + mainSize/2).attr("y", height - 10).attr("text-anchor", "middle").style("font-size", "12px").text("Dataset Noise Ceiling (r)");
    svg.append("text").attr("transform", "rotate(-90)").attr("x", -(margin.top + mainSize/2)).attr("y", centerXOffset + 25).attr("text-anchor", "middle").style("font-size", "12px").text("Normalized Performance Gap");

  }, [dimensions, aggregatedPoints, roi, hoveredData, selectedPoint]); // 监听 hover 和 select
    ...
    <div style={{ display: "flex", width: "100%", height: "100%", background: "#fafafa", borderRadius: 12, overflow: "hidden" }}>
      ...
      <div style={{ width: "260px", borderLeft: "1px solid #eee", padding: "20px", backgroundColor: "#fff", display: "flex", flexDirection: "column", gap: "15px" }}>
        ...
        {displayInfo ? (
          ...
          <div style={{ color: "#999", fontSize: "12px", textAlign: "center", marginTop: "40px", fontStyle: "italic" }}>
            Hover or click to see performance fission
          </div>
        )}
      </div>
    </div>
